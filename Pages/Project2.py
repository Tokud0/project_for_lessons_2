...
class Project2:
    ...
    def app(self):
        ...
        sourse_option = st.selectbox(
            "Pick up sourse of the video stream",
        ...
        if sourse_option == "Mobile camera":
            img_file = st.camera_input("Take a photo")
        elif sourse_option == "Youtube link":
        ...
        elif sourse_option =="Local drive":
        ...
        if sourse_option == "Mobile camera" and img_file is not None:
            file_bytes = np.asarray(bytearray(img_file.read()), dtype=np.uint8)
            frame = cv2.imdecode(file_bytes, 1)
            st.image(frame, channels="BGR")

        elif run_button and video_url is not None and sourse_option in ["Local drive", "RTSP"]:
